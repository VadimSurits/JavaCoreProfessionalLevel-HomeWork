...
public class Server {
    ...
    public Server() {
        ...
        /// На мой взгляд наиболее подходящий вариант ExecutorService для чата - это CachedThreadPool, поскольку
        /// у нас неопределенное число пользователей, и здесь максимально полезной будет опция автоматического
        /// создания необходимого числа потоков для новых пользователей, также как и автоматическое удаление
        /// (через 60 секунд) тех лишних потоков, которые были созданы ранее, но в текущий момент не используются.
